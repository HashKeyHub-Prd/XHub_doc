...
2、判断该钱包状态是否正常，正常状态下可点击，若钱包状态为暂停使用或冻结，则“充值”按钮置灰，不可点击，若此时有资产转入，则会进入冻结//Frozen状态
...
2、判断该钱包状态是否正常，正常状态下可点击，若钱包状态为暂停使用或冻结，则“提现”按钮置灰
...
* 交互逻辑
  * 地址搜索框
    * 点击搜索框后，清空默认提示语，限制50个字符内，超过则无法输入，根据输入的内容进行地址或标签的模糊搜索
...
* 交互逻辑
  ...
  * 校验成功后，按以下条件进行判断：
    ...
    * 若From钱包为自定义钱包且设置风了控策略，①风控策略为“直接拒绝”，则转账失败，Toast提示“转账失败//Transfer failed”，订单状态更新为Failed；②风控策略为“人工审核”，则Toast提示“转账申请审核中//Transfer application under review”,订单状态更新为Awaiting approva,若审核结果为通过，则订单状态更新为Done，From钱包可用余额减少，To钱包可用余额增加；若审核结果为拒绝，则订单状态更新为Terminated，转账失败
...
* 交互逻辑
  * 币种搜索//search Coin
    * 点击搜索框后，清空默认提示语，仅限输入字母（大小写不限）与数字，限制10个字符内，超过则无法输入，根据输入的内容进行币种的模糊搜索
    * 点击“取消//Cancel”后则清空搜索框内容 
  * 类型//Type
