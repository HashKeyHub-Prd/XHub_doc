...
* 谷歌验证码/谷歌验证码+钱包密码校验成功后，按以下顺序进行校验：
    ...
    * 以上条件校验通过后，才会生成提现订单，若钱包为自定义钱包且设置风了控策略，策略为“人工审核”，则Toast提示“提现申请审核中//Withdraw application under review”，页面跳转至当前钱包【交易//Transaction】页面
...
* 页面元素
  ...
  * 交易列表
    * 顺序：按照交易时间由近及远进行排序
    * 列表内容：币种简称、交易日期（精确到秒）、交易类型(充值//Deposit、提现//Withdraw、内部转账//Transfer、授信//Credit)、交易数量（+代表钱包入金，用绿色表示；-代表钱包出金，用红色表示）、手续费//Fee、订单状态
    * 状态枚举

      已确认//Done：已成功完成的订单状态，绿色圆点标识。

      待确认//Pending：正在进行中的订单状态。对于充值/提现订单需加上区块确认数，例如：pending (当前确认数/订单完成所需确认数），黄色圆点标识。
      
      待审批//Awaiting approval：对于机构账户的自定义钱包，如果设置了审批策略，出金请求（提现和划出）发起后，订单状态为“awaiting approval"，黄色圆点标识。
      
      已隔离//Segregated：对于违反了AML的充值, 运营人员审核拒绝后，充值订单状态变成“segregated"，红色圆点标识。
      
      已拒绝//Terminated：对于违反了AML的提现, 运营人员审核拒绝后，提现订单状态变成“terminated”。对于“awaiting approval"状态的订单，在用户审批拒绝后，状态变为"terminated"，红色圆点标识。
      
      失败//Failed：订单失败状态，红色圆点标识。
* 交互逻辑
  * 币种搜索//Search coin
